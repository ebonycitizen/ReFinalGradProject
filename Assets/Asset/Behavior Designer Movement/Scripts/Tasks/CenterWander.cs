...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    ...
    public class CenterWander : NavMeshMovement
    {
        ...
        private Vector3 m_initPostion = Vector3.zero;

        public override void OnStart()
        {
            m_initPostion = transform.position;
        }
        ...
        {
#if UNITY_EDITOR
            Gizmos.DrawWireSphere(m_initPostion, minWanderDistance.Value);
#endif
        ...
        private bool TrySetTarget()
        {
            ...
            var destination = m_initPostion;
            while (!validDestination && attempts > 0)
            {
                direction = direction + Random.insideUnitSphere * wanderRate.Value;
                destination = m_initPostion + direction.normalized * Random.Range(minWanderDistance.Value, maxWanderDistance.Value);
