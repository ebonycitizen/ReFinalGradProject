...
namespace BoidECSCoral
{
    ...
    [AlwaysUpdateSystem]
    [UpdateBefore(typeof(BoidsSimulationSystemCoral))]
    public class BoidsEntityGenerationSystemCoral: ComponentSystem
    {
        ...
        protected override void OnUpdate()
        {
            ...
            for (int i = 0; i < BootstrapCoral.Boid.count - entities.Length; ++i)
            {
                if (hasSpawn)
                    break;
                CreateEntity(i);
            ...
            {
                PostUpdateCommands.SetComponent(entities[i], new TargetPos { Value = BootstrapCoral.SetSpawnTarget(i) });
            }

            if (BootstrapCoral.Boid.hasDestroy)
            {
                for (int i = 0; i < entities.Length; i++)
                {
                    PostUpdateCommands.DestroyEntity(entities[i]);
        ...
        {
            var scale = BootstrapCoral.Boid.scale;
            var renderer = BootstrapCoral.Boid.renderer;
            var initSpeed = BootstrapCoral.Param.initSpeed;

            PostUpdateCommands.CreateEntity(archetype);
            PostUpdateCommands.SetComponent(new Position { Value = BootstrapCoral.SetSpawnTarget(i) + random.NextFloat3(1f) });
            ...
            PostUpdateCommands.SetComponent(new TargetPos { Value = BootstrapCoral.SetSpawnTarget(i) });

            renderer.material = BootstrapCoral.SetSpawnMat(i);
            PostUpdateCommands.SetSharedComponent(renderer);
        }
    }

    public class BoidsSimulationSystemCoral : JobComponentSystem
    {
        ComponentGroup group;
        ...
        //[BurstCompile]
        //public struct WallJobCoral : IJobProcessComponentData<Position, Acceleration>
        //{
        ...
        protected override JobHandle OnUpdate(JobHandle inputDeps)
        {
            ...
            {
                prodThresh = math.cos(math.radians(BootstrapCoral.Param.neighborFov)),
                distThresh = BootstrapCoral.Param.neighborDistance,
                neighborsFromEntity = GetBufferFromEntity<NeighborsEntityBuffer>(false),
                positionFromEntity = GetComponentDataFromEntity<Position>(true),
                entities = group.GetEntityArray(),
            };

            //var wall = new WallJobCoral
            //{
            //    scale = BootstrapCoral.Param.wallScale * 0.5f,
            //    thresh = BootstrapCoral.Param.wallDistance,
            //    weight = BootstrapCoral.Param.wallWeight,
            //    selfPos = BootstrapCoral.Boid.selfTransform.position,
            //};

            var separation = new SeparationJob
            {
                separationWeight = BootstrapCoral.Param.separationWeight,
            ...
            {
                alignmentWeight = BootstrapCoral.Param.alignmentWeight,
                neighborsFromEntity = GetBufferFromEntity<NeighborsEntityBuffer>(true),
            ...
            {
                cohesionWeight = BootstrapCoral.Param.cohesionWeight,
                neighborsFromEntity = GetBufferFromEntity<NeighborsEntityBuffer>(true),
                positionFromEntity = GetComponentDataFromEntity<Position>(true),
            };
            ...
            {
                dt = Time.deltaTime,
                minSpeed = BootstrapCoral.Param.minSpeed,
                maxSpeed = BootstrapCoral.Param.maxSpeed,
            };

            var toTarget = new ToTargetJob
            {
                targetWeight = BootstrapCoral.Param.targetWeight,
            };
